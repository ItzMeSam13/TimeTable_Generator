...
export const GetUpcomingTask = async (req, res) => {
    try {
        ...
        if (!calendarEvents.items || calendarEvents.items.length === 0) {
            return res.status(404).json({ error: "No upcoming tasks found." });
        }

        const taskEvents = calendarEvents.items.filter(event =>
            event.description && event.description.includes("Generated by AI")
        );

        if (taskEvents.length === 0) {
            return res.status(404).json({ error: "No upcoming synced tasks found." });
        }

        const nearestTask = taskEvents[0];

        return res.status(200).json({
            message: "Upcoming task retrieved successfully",
            task: {
                title: nearestTask.summary,
                startTime: nearestTask.start.dateTime || nearestTask.start.date,
                endTime: nearestTask.end.dateTime || nearestTask.end.date,
